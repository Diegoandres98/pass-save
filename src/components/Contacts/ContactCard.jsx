...
import { getContactCategoryById } from '../../lib/contactCategories';

export default function ContactCard({ entry, onEdit, onDelete }) {
  const [copied, setCopied] = useState(null);

  const category = getContactCategoryById(entry.category);
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span
                className={`${category.color} text-white text-xs px-2 py-0.5 rounded-full flex-shrink-0`}
              >
                {category.label}
              </span>
            </div>
          ...
          <div className="mt-3 space-y-2">
            <div className="flex items-center gap-2">
              <span className="text-gray-500 text-sm w-16">Tel.</span>
              <a
            ...
            {entry.notes && (
              <div className="flex items-start gap-2">
                <span className="text-gray-500 text-sm w-16">Notas</span>
                <span className="text-gray-400 text-sm flex-1 whitespace-pre-wrap">
                  {entry.notes}
                </span>
              </div>
            )}
